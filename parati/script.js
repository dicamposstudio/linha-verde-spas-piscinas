...
// Substitua o número abaixo pelo WhatsApp oficial antes de publicar.
...
function atualizarLinksWhatsApp() {
  ...
  links.forEach((link) => {
    ...
    link.addEventListener('click', () => {
      ...
      window.dataLayer.push({ event: 'click_whatsapp', campaign: 'piscina_parati_gerador_cloro' });
      if (window.fbq) window.fbq('trackCustom', 'ClickWhatsAppParati');
      if (window.gtag) window.gtag('event', 'click_whatsapp', { campaign_name: 'piscina_parati_gerador_cloro' });
    });
  });
}
...
function gerenciarPromocaoTemporaria() {
  ...
  window.dataLayer.push({
    event: promocaoAtiva ? 'promocao_parati_ativa' : 'promocao_parati_encerrada',
